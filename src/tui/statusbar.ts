 ...
 * a light green (unless `color` is false). Uses the 256-colour palette's light
 * green (colour 120) — a touch paler than bright green while still a palette
 * colour rather than a hardcoded RGB. The returned string has no trailing
 * newline; the caller positions it.
 */
export function renderStatusBar(info: BarInfo, cols: number, opts: { color?: boolean } = {}): string {
  ...
  return `${ESC}[30;48;5;120m${line}${ESC}[0m`;
}
